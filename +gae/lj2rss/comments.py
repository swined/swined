...
class StatsPage(RequestHandler):
	...
	r = []
	    ...
	    q = db.Query(Request)
	    q.filter('time > ', datetime.datetime.now() - datetime.timedelta(minutes = i * 10))
	    q.filter('time < ', datetime.datetime.now() - datetime.timedelta(minutes = (i - 1) * 10))
	    r.append(str(q.count() / 10))
	r.reverse()
	return r
    def stats1(self):
	r = []
	...
	c = 'cht=lc&chs=800x160&chg=4,25&chxt=y&chxr=0,0,' + str(int(m)) + '&chd=t:' + ','.join(S)
	self.response.out.write('<meta http-equiv="REFRESH" content="30;URL=/stats.html"></meta>')
